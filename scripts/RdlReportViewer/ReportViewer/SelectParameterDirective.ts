module RdlReportViewer.Directives {
    export var selectParameterDirective = [() => {
        return {
            ...
            template: '<label data-ng-bind="parameter.label"></label><ui-select multiple ng-model="selection.value" theme="bootstrap"><ui-select-match>{{$item.label}}</ui-select-match><ui-select-choices repeat="opt in parameter.validValues | filter:$select.search">{{opt.label}}</ui-select-choices></ui-select>',
            //="parameter.multiple"
